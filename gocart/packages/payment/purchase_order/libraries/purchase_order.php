...
class purchase_order
{
	...
	there is also the possibility that this payment method is not approved for this purchase. In that case, it should return a blank array 
	*/
	
	//these are the front end form and check functions
	function checkout_form($post = false)
	{
		$settings	= $this->CI->Settings_model->get_settings('purchase_order');
		...
		if($enabled)
		{
			...
			$form['form'] = $this->CI->load->view('purchase_order', '', true);
		}
	...
	function description()
	{
		...
		*/
		...
		return 'Purchase order: ' . $_POST['ponumber'];
	...
	function install()
	{
		...
		$this->CI->Settings_model->save_settings('purchase_order', array('enabled'=>'0'));
	}
	
	function uninstall()
	{
		$this->CI->Settings_model->delete_settings('purchase_order');
	}
	...
	{	
		...
		else
		{
			...
			$this->CI->Settings_model->save_settings('purchase_order', array('enabled'=>$_POST['enabled']));
